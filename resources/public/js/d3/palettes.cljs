...
(defn gradual [n c1 c2]
  (.. js/d3.scale linear (domain (clj->js [0 n])) (range (clj->js [c1 c2]))))

(defonce color-lovers (atom nil))

(do @color-lovers)

(when-not @color-lovers
  (cl/top-palettes #(swap! color-lovers assoc :top %))
  (cl/new-palettes #(swap! color-lovers assoc :new %)))
